...
expense_tracker::Date ExpenseManager::parseDateString(const std::string& dateStr) const {
    std::istringstream iss(dateStr);
    date::year_month_day ymd;

    // Attempt to parse the date string using the specified format.
    // date::parse will move the stream iterator (iss) past the successfully parsed part.
    iss >> date::parse("%Y-%m-%d", ymd);

    // Condition 1: Basic parsing failure (e.g., "abc", "2023/01/01")
    // `iss.fail()` will be true if the input did not match the format at all.
    // Condition 2: Date is invalid according to calendar rules (e.g., "2023-02-30", "2023-00-10")
    // `!ymd.ok()` will be true for dates like February 30th.
    if (iss.fail() || !ymd.ok()) {
        return expense_tracker::Date{0, 0, 0}; // Return an invalid Date
    }

    // Condition 3: Ensure there are no trailing non-whitespace characters.
    // After a successful parse of "YYYY-MM-DD", `iss` might point to subsequent characters.
    // We try to read another character. If successful, it means there was extra data.
    char trailing_char;
    if (iss >> trailing_char) { // `iss >> char` skips leading whitespace then reads a char.
        return expense_tracker::Date{0, 0, 0}; // Extra non-whitespace characters found after date
    }

    // If all checks pass, the date is valid and strictly adheres to "YYYY-MM-DD" followed by only optional whitespace.
    return expense_tracker::Date{
