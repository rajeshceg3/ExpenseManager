...
#include "date/date.h" // Add if not present (likely needed directly now)
...
expense_tracker::Date getDateInput(const std::string& prompt) {
    std::string dateStr;
    date::year_month_day ymd; // Use the date library's type for parsing

    while (true) {
        fmt::print(prompt); // e.g., "Enter date (YYYY-MM-DD): "
        ...
        if (iss.fail() || !ymd.ok()) {
            fmt::print("Invalid date format or value (e.g., '2023-02-30' is invalid).\n");
            fmt::print("Please use YYYY-MM-DD format: ");
            continue; // Loop to ask again
        }

        // Check for trailing characters
        char trailing_char;
        if (iss >> trailing_char) { // If any non-whitespace char is left
            fmt::print("Invalid format. Extra characters found after date.\n");
            fmt::print("Please use YYYY-MM-DD format: ");
            continue; // Loop to ask again
        }
    ...
    // Convert validated date::year_month_day to our simple Date struct
...
void addExpenseUI(expense_tracker::ExpenseManager& manager) {
    std::string description, category, typeStr;
    double amount;
    ...
    do {
        ...
        switch (choice) {
            case 1:
                ...
                break;
            case 2: {
                const auto& expenses = manager.getAllExpenses(); // Get all expenses
                printExpenses(expenses, "--- All Expenses ---");   // Use the local printExpenses function
                break;
            }
            case 3:
